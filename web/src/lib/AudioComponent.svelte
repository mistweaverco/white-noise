<script lang="ts">
  ...
  const toggleAudio = (assetIdx: number) => {
    ...
    const fileIdxsToggleLater = filesCount > 1
...
<div class="loading loading-spinner text-success size-max center {isLoading ? '' : 'hidden'}">Loading audio assets...</div>
      ...
      <div class="card-body">
        <fieldset class="fieldset">
          <div class="tooltip" data-tip="Volume">
            <input type="range" value="1" min="0" max="1" step="0.01" on:input={onVolumeChange} bind:this={volumeElement} />
          </div>
          <!-- Audio Buttons, flex row -->
          <div class="flex flex-row flex-wrap gap-2 mt-4 align-center justify-center">
            {#each audioAssets as asset, assetIdx}
              <div class="tooltip" data-tip="{asset.name}">
                <button class="btn btn-soft" on:click={() => toggleAudio(assetIdx)}>
                  <span class="{audioPlayersPlaybackStates[assetIdx] === AudioAssetPlaybackState.PAUSED ? 'hidden' : 'loading loading-bars'}"></span>
                  {asset.icons.string}
                </button>
              </div>
              {#each asset.files as file, fileIdx}
                <audio preload="none" loop class="hidden" data-asset-idx="{assetIdx}" data-file-idx="{fileIdx}">
